...
@Component({
  selector: 'app-description-grades-view',
  templateUrl: './description-grades-view.component.html',
  styleUrls: ['./description-grades-view.component.css']
})
export class DescriptionGradesViewComponent implements OnInit {
  tab = 'description';
        ...
        if(data) {
          ...
          Object.keys(this.fullData.controls).forEach(key => {
            if (this.fullData.get(key) instanceof FormControl && description[key]) {
              ...
              this.fullData.get(key).disable();
            }
          });

          // this.fullData.disable();
        }
        // console.log(data);
      })
    })
