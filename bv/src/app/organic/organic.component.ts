...
export class OrganicComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activatedroute.data.subscribe(data => {
      ...
      if (data.download) {
        this.service.getOrganic(this.id).subscribe(response => {
...
// @ts-ignore
  ...
  edit() {
    ...
    this.router.navigate(['/admin/organic-edit/' + this.activatedroute.snapshot.params.id]);
  }
  delete() {
    if (this.langService.editable) {
      return;
    }
    this.service.deleteOrganic(this.activatedroute.snapshot.params.id).subscribe(() => {
  ...
  returnBack() {
    ...
    this.router.navigate(['/admin/organic/' + this.activatedroute.snapshot.params.id]);
  }
  submit() {
    if (this.langService.editable) {
      return;
    ...
    if (!this.activatedroute.snapshot.data.download) {
      observer = this.service.addOrganic(this.form.value);
    } else {
      observer = this.service.editOrganic(this.form.value, this.id);
    }
    observer.subscribe(data => {
      if (this.image.isImageChanged) {
        this.service.setImage(this.STORAGE, data.id, this.image.getNewImage()).subscribe(() => {

          this.router.navigate(['/admin/organic/' + data.id]);
        });
      } else {
        this.router.navigate(['/admin/organic/' + data.id]);
      }
